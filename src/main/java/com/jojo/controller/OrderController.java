...
public class OrderController {
    ...
    public ResultVo add(@RequestBody OrderAddVo orderAddVo,HttpSession session){
        ResultVo resultVo = orderService.addOrder(orderAddVo.getIds(),orderAddVo.getTotalMoney(),(User)session.getAttribute("loginUser"));
        System.out.println(resultVo);
        return resultVo;
    }

    @RequestMapping("/getAddressByUid")
    public ResultVo getAddressByUid(HttpSession session){
        ResultVo resultVo = orderService.getAddressByUid((User)(session.getAttribute("loginUser")));
        return resultVo;
    }
